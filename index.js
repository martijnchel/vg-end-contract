...
// VEILIGHEID: op false = testen, stuurt NIETS. Zet op true als aantallen kloppen.
...
async function runRetentionBot() {
    console.log(`--- [START] BATCH SCAN (T+50 tot T+65) - MAX 30 ---`);
            ...
            for (const member of members) {
                ...
                if (contract) {
                    ...
                    if (visits.length >= MIN_VISITS_RECENT) {
                        ...
                        sentCount++;
                        console.log(`[${sentCount}/30] ${ECHT_VERSTUREN ? 'Verzonden' : 'ZOU STUREN'}: ${member.firstname} ${member.lastname} (${member.email}) | ${contract.membership_name} | eind ${contract.contract_end_date} | ${visits.length} bezoeken`);
                        await sleep(3000);
                    }
                }
            }
